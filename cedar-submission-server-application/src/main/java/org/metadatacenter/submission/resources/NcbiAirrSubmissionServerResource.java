...
import org.metadatacenter.submission.ncbi.airr.AIRRTemplateInstance2SRAXMLConverter;
...
import org.metadatacenter.submission.ncbi.airr.NcbiAirrSubmissionXMLFileGenerator;
...
public class NcbiAirrSubmissionServerResource extends CedarMicroserviceResource {
  ...
  private final AIRRTemplateInstance2SRAXMLConverter airrTemplate2SRAConverter;
  private final NcbiAirrSubmissionXMLFileGenerator ncbiSubmissionXMLFileGenerator;

  public NcbiAirrSubmissionServerResource(CedarConfig cedarConfig) {
    super(cedarConfig);
    this.bioSampleValidator = new BioSampleValidator();
    this.airrTemplate2SRAConverter = new AIRRTemplateInstance2SRAXMLConverter();
    this.ncbiSubmissionXMLFileGenerator = new NcbiAirrSubmissionXMLFileGenerator();
  }

  public static void injectServices(NcbiSubmissionQueueService ncbiSubmissionQueueService) {
    NcbiAirrSubmissionServerResource.ncbiSubmissionQueueService = ncbiSubmissionQueueService;
  }
  ...
  public Response validate(AIRRTemplate airrInstance) throws CedarException {
    ...
    try {
      String bioSampleSubmissionXML = this.airrTemplate2SRAConverter
          .convertTemplateInstanceToXML(airrInstance);
        ...
        if (SubmissionUploadManager.getInstance().isSubmissionUploadComplete(data.getSubmissionId())) {
          ...
          NcbiSubmission ncbiSubmission =
              NcbiSubmissionUtil.generateSubmission(data.getSubmissionId(), userId, ncbiFolderName, this
                  .ncbiSubmissionXMLFileGenerator);
